...
public final class MemoryIndexObjectView extends FuncListDisp implements AnChangeListener {
  // Constructor
  public MemoryIndexObjectView(final int type, final int subtype, final String help_id) {
    super(AnWindow.getInstance(), type, subtype, help_id);
    setAccessibility(AnLocale.getString("Memory or Index Object"));
  ...
  public void stateChanged(AnChangeEvent e) {
    //        System.out.println("MemoryIndexObjectView stateChanged:  " + e.getType());
    switch (e.getType()) {
      ...
      case SETTING_CHANGED:
        ...
        if (anSettingChangeEvent.getType() == AnSettingChangeEvent.Type.COMPARE_MODE
            || anSettingChangeEvent.getType() == AnSettingChangeEvent.Type.VIEW_MODE
  ...
  /**
   * Called before new memory/index objects are created. Cleanup old ones before they are removed.
   */
  public void toBeRemoved() {
    // Remove listeners
    AnEventManager.getInstance().removeListener(this);
  }
