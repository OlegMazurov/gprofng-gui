...
public final class FunctionsView extends FuncListDisp implements AnChangeListener {

  public FunctionsView() {
    super(AnWindow.getInstance(), AnDisplay.DSP_Functions, 0, AnVariable.HELP_TabsFunctions);
    setAccessibility(AnLocale.getString("Functions"));
  ...
  public void stateChanged(AnChangeEvent e) {
    //        System.out.println("FunctionsView stateChanged:  " + e.getType());
    switch (e.getType()) {
      ...
      case SELECTION_UPDATE:
        ignoreSelected = true;
        computeOnAWorkerThread(); // This will recompute if necessary and fire a SELECTION_CHANGED
        // event.
        break;
  ...
  public void setComputed(boolean set) {
    ...
    AnWindow.getInstance().getCalledByCallsFunctionsView().setComputed(set);
  }

  @Override
  protected void setAvailable(boolean set) {
    super.setAvailable(set);
    AnWindow.getInstance().getCalledByCallsFunctionsView().setAvailable(set);
  }

  /**
   * Ensure Functions View has been computed (even if not selected) Note: must directly or
   * indirectly be called from another view's doCompute
   */
  public void computeIfNeeded() {
    AnUtility.checkIfOnAWTThread(false);
    ignoreSelected = true;
    updateSummary = false;
    boolean savedComputed = computed;
    doCompute();
